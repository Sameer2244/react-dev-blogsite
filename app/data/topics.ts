// ─── Topic Data Database ────────────────────────────────────────────────────
// Content extracted and expanded from Remotion video compositions (Day 003–008)
...
// ─── COLOR PALETTE (from Remotion video compositions) ───────────────────────
const CYAN = "#38bdf8";
const VIOLET = "#d2a8ff";
...
export const topics: Topic[] = [
  // ─── DAY 003: Virtual DOM vs Real DOM ─────────────────────────────────────
  {
    slug: "virtual-dom-vs-real-dom",
    day: 3,
    title: "Virtual DOM vs Real DOM — What Actually Happens",
    ...
    sections: [
      ...
      },
      {
        ...
        badgeColor: EMERALD,
        body: "React solves this by keeping a lightweight copy of the DOM in memory, called the Virtual DOM. It is just a plain JavaScript object tree, meaning updates are virtually free — no browser rendering, no style recalculation, just fast in-memory object manipulation.",
        code: {
      ...
      },
      {
        ...
        badgeColor: AMBER,
        body: "When state changes, React builds a brand new Virtual DOM, then compares it with the previous snapshot using its reconciliation diffing algorithm. This comparison identifies the exact minimum set of changes needed — not a full tree rebuild, but surgical node-level updates.",
      },
      {
    ...
    keyTakeaways: [
      ...
      "The Virtual DOM is a lightweight JavaScript object — updates are instant.",
  ...
  // ─── DAY 004: useEffect Cleanup ────────────────────────────────────────────
  {
    slug: "useeffect-cleanup-function",
    day: 4,
    title: "useEffect Cleanup Function — The Bug Everyone Misses",
    ...
    category: "Hooks",
    description:
      "The cleanup function — the bug that slips past most React developers and causes serious memory leaks. Learn exactly when useEffect runs, why cleanup is non-optional, and how to fix the most common leak patterns.",
    sections: [
      {
        heading: "What useEffect Does",
        badge: "What useEffect Does",
        badgeColor: CYAN,
        body: "useEffect runs after every render. You use it for subscriptions, timers, event listeners, and fetching data. These are all side effects — things that reach outside the React component tree. And that's exactly what makes cleanup so critical.",
        code: {
      ...
      },
      {
        ...
        badge: "The Bug",
        badgeColor: ROSE,
        body: "When a component unmounts — or when deps change — the effect runs again. If you don't clean up the previous effect, you get stacked subscriptions, multiple timers firing simultaneously, and event listeners that reference unmounted components. This is the memory leak.",
        code: {
          filename: "broken.tsx",
          accentColor: ROSE,
          lines: [
            [
              { text: "// ❌ Memory leak — subscription never cleaned up", color: ROSE },
            ],
            [
            ...
            [
              { text: "}, []);", color: "#e1e4e8" },
              { text: "  // ← no return! leak!", color: ROSE },
            ],
          ],
        },
      },
      {
        ...
        code: {
          ...
          lines: [
            [
              { text: "// ✅ Clean — subscription removed on unmount", color: EMERALD, glow: true },
            ],
            [
              { text: "useEffect", color: VIOLET },
              { text: "(() => {", color: "#e1e4e8" },
    ...
    keyTakeaways: [
      ...
      "The return function from useEffect is the cleanup — always return one for async resources.",
  ...
  // ─── DAY 005: useState vs useRef ───────────────────────────────────────────
  {
    ...
    category: "Hooks",
    description:
      "Two hooks. Both store values. But they work completely differently — and using the wrong one causes real bugs. Learn exactly when to reach for useState vs useRef, with a practical decision rule you can apply immediately.",
    sections: [
      {
        heading: "useState — Reactive Storage",
        badge: "useState — Reactive Storage",
        badgeColor: CYAN,
        body: "useState stores a value and re-renders your component every time it changes. That re-render is what keeps your UI in sync. If the value should appear on screen — useState is your hook. React tracks it, diffs it, and repaints when it changes.",
        code: {
          ...
          lines: [
            ...
            [{ text: "", color: "#e1e4e8" }],
            [{ text: "// ✅ triggers a re-render", color: CYAN }],
            [
      ...
      {
        heading: "useRef — Silent Storage",
        badge: "useRef — Silent Storage",
        badgeColor: VIOLET,
        body: "useRef also stores a value that persists across renders. But changing a ref never triggers a re-render. The component keeps running — React has no idea. This makes it perfect for values you need to track internally without affecting the UI.",
        code: {
          filename: "Timer.tsx",
          accentColor: VIOLET,
          lines: [
            ...
            [{ text: "", color: "#e1e4e8" }],
            [{ text: "// ✅ persists across renders", color: VIOLET }],
            [
              { text: "timerRef", color: "#e1e4e8" },
              { text: ".", color: "#ff7b72" },
            ...
            [{ text: "", color: "#e1e4e8" }],
            [{ text: "// ❌ does NOT trigger re-render", color: ROSE }],
          ],
        },
      },
      {
        ...
        badgeColor: AMBER,
        body: "You never display a timer ID on screen. Storing it in useState triggers a re-render on every setInterval call. useRef holds it silently — your component stays fast. This is the canonical use case that shows exactly when each hook belongs.",
        code: {
          filename: "correct.tsx",
          accentColor: EMERALD,
          lines: [
            [{ text: "// ✅ Correct — no wasted renders", color: EMERALD, glow: true }],
            [
              { text: "const ", color: "#ff7b72" },
              { text: "timerRef", color: "#e1e4e8" },
              { text: " = ", color: "#ff7b72" },
    ...
    keyTakeaways: [
      "useState is reactive — changing it triggers a re-render. React knows.",
      "useRef is invisible — changing it never triggers a re-render. React is unaware.",
  ...
  // ─── DAY 006: Controlled vs Uncontrolled ───────────────────────────────────
  {
    ...
    category: "Patterns",
    description:
      "React forms: do you use useState or useRef? The choice between Controlled and Uncontrolled components fundamentally changes how your form data flows through the application — and which approach you pick determines performance, testability, and complexity.",
    sections: [
      {
        heading: "Controlled — React Owns the Value",
        badge: "Controlled — React State",
        badgeColor: EMERALD,
      ...
      {
        heading: "Uncontrolled — The DOM Owns the Value",
        badge: "Uncontrolled — DOM Ref",
        badgeColor: CYAN,
        body: "In an uncontrolled component, the DOM itself manages the input state. You use useRef to read the value only when you need it — typically on form submit. There are no re-renders on every keystroke. This is faster for large forms, and simpler for file inputs.",
        code: {
  ...
  // ─── DAY 007: React Keys ───────────────────────────────────────────────────
  {
    ...
    day: 7,
    title: "React Keys — Why Index as Key Breaks Your App",
    ...
    category: "Performance",
    description:
      "You use index as a key and it looks fine. Then items reorder — and React silently corrupts your UI. Learn why keys are identity tags, not just performance hints, and exactly when index-as-key destroys component state.",
    sections: [
      {
        heading: "The key Prop — React's Identity Tag",
        badge: "The key Prop — Identity Tag",
        badgeColor: ORANGE,
        body: "The key prop tells React which list item is which across renders. It is not a performance hint — it is an identity label. React uses keys during reconciliation to decide whether to reuse, update, or destroy a component instance. Stable keys → stable component state.",
        code: {
          filename: "list.tsx",
          accentColor: ORANGE,
          lines: [
            [{ text: "// ✅ Correct — stable unique ID", color: "#8b949e" }],
            [
              { text: "{", color: "#e1e4e8" },
      ...
      },
      {
        ...
        badgeColor: ROSE,
        body: "When you use array index as the key, React maps component identity to position. If items reorder or prepend, the index shifts — but the key stays the same positional number. React thinks the component at position 0 is the same component as before, so it reuses its state. But it isn't. Your internal state now belongs to the wrong item.",
        code: {
          filename: "broken.tsx",
          accentColor: ROSE,
          lines: [
            [{ text: "// ❌ Bug — index shifts on reorder", color: ROSE }],
            [
              { text: "{", color: "#e1e4e8" },
              { text: "items", color: "#79c0ff" },
              { text: ".map((", color: "#e1e4e8" },
            ...
            [
              { text: "  // ↑ state leaks to wrong item!", color: ROSE },
            ],
            [
              { text: "))}", color: "#e1e4e8" },
            ],
    ...
    keyTakeaways: [
      "Keys are identity labels, not performance hints — React uses them during reconciliation.",
  ...
  // ─── DAY 008: SSR vs SSG vs ISR vs CSR ─────────────────────────────────────
  {
    slug: "ssr-ssg-isr-csr",
    day: 8,
    title: "SSR · SSG · ISR · CSR — When Does Your Page Get Built?",
    shortTitle: "SSR · SSG · ISR · CSR",
    badge: "Day 008",
    accentColor: BLUE,
    ...
    category: "Rendering",
    description:
      "Four rendering strategies. One big question: when does your page actually get built? Pick wrong — and you sacrifice speed or freshness. This is the single most impactful architectural decision you make in a Next.js app.",
    sections: [
      {
        heading: "CSR — Client-Side Rendering",
        badge: "CSR — Client Side",
        badgeColor: AMBER,
        ...
        code: {
          ...
          accentColor: AMBER,
          lines: [
            [{ text: "// CSR — data fetched after page load", color: "#8b949e" }],
            [
              { text: "const ", color: "#ff7b72" },
      ...
      },
      {
        ...
        badge: "SSG — Build Time",
        badgeColor: EMERALD,
        body: "Pages are built once at deploy time and served as static HTML. Blazingly fast — content is on a CDN edge node near every user. Perfect for blogs, marketing pages, and documentation that changes rarely. Zero server cost, maximum speed.",
        code: {
          filename: "BlogPost.tsx",
          accentColor: EMERALD,
          lines: [
            [{ text: "// SSG — built once at deploy time", color: "#8b949e" }],
            [
      ...
      },
      {
        ...
        code: {
          ...
          accentColor: BLUE,
          lines: [
            [{ text: "// SSR — runs on every request", color: "#8b949e" }],
            [
